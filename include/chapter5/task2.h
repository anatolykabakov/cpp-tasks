 ...
 * @details Write th eprogram with class for realization complex number.
...
class ComplexNumber {
 public:
  ...
  ComplexNumber operator/(const ComplexNumber& obj) {
    const double re = ((x_ * obj.get_real()) + (y_ * obj.get_immediance())) /
                      (std::pow(obj.get_real(), 2) + std::pow(obj.get_immediance(), 2));
    const double im = ((obj.get_real() * y_) - (x_ * obj.get_immediance())) /
                      (std::pow(obj.get_real(), 2) + std::pow(obj.get_immediance(), 2));
