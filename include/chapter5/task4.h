/**
 * @brief Chapter 5, task 4.
 * @details Write the program with class for describe vehicle.
...
class Vehicle {
 public:
  Vehicle() = delete;
  Vehicle(const std::string &model, const std::string &color,
          const double fuel_tank_volume, const double norm_consumption_gasoline)
      : model_(model),
        color_(color),
        fuel_tank_volume_(fuel_tank_volume),
...
class Truck : public Vehicle {
 public:
  Truck(const std::string &model, const std::string &color, double fuel_tank_volume,
        double norm_consumption_gasoline, double carrying_capacity)
      : Vehicle(model, color, fuel_tank_volume, norm_consumption_gasoline),
